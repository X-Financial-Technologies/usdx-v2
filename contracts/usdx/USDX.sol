...
import "contracts/USDX/blocklist/BlocklistClientUpgradeable.sol";
import "contracts/USDX/allowlist/AllowlistClientUpgradeable.sol";
  ...
  SanctionsListClientUpgradeable
{
  ...
  function _beforeTokenTransfer(
    ...
    if (from != msg.sender && to != msg.sender) {
      require(!_isBlocked(msg.sender), "USDX: 'sender' address blocked");
      require(!_isSanctioned(msg.sender), "USDX: 'sender' address sanctioned");
      require(
        _isAllowed(msg.sender),
        "USDX: 'sender' address not on allowlist"
      );
    }

    if (from != address(0)) {
      // If not minting
      require(!_isBlocked(from), "USDX: 'from' address blocked");
      require(!_isSanctioned(from), "USDX: 'from' address sanctioned");
      require(_isAllowed(from), "USDX: 'from' address not on allowlist");
    }

    if (to != address(0)) {
      // If not burning
      require(!_isBlocked(to), "USDX: 'to' address blocked");
      require(!_isSanctioned(to), "USDX: 'to' address sanctioned");
      require(_isAllowed(to), "USDX: 'to' address not on allowlist");
    }
  }
